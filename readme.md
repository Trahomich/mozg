...
**1943 — модель МакКаллока–Питтса.** Уоррен МакКаллок и Уолтер Питтс предложили первую математическую модель нейрона: бинарный пороговый элемент. Доказали, что сеть из таких нейронов может вычислить любую логическую функцию. Первая формальная связь между нейробиологией и вычислениями.

**1958 — перцептрон Розенблатта.** Фрэнк Розенблатт (Cornell Aeronautical Laboratory) создал первую обучаемую нейронную сеть — Mark I Perceptron. Это была не программа, а физическая машина: 5 тонн, 400 фотодетекторов, 512 настраиваемых весов на потенциометрах, моторы и реле. Перцептрон учился распознавать простые изображения на 20×20 пикселях. New York Times написала, что через год машина будет «ходить, говорить, видеть и размножаться». Не смогла.

Перцептрон — это один искусственный нейрон с обучаемыми весами: взвешенная сумма входов → пороговая функция. Один перцептрон может выучить линейно разделимую задачу (И, ИЛИ), но не XOR.

**1969 — «Перцептроны» Минского и Пейперта.** Доказали, что однослойный перцептрон не может вычислить XOR. Книга убила финансирование нейросетей на 15 лет — «AI зима».

**1986 — backpropagation.** Румельхарт, Хинтон и Уильямс популяризировали алгоритм обратного распространения ошибки. Многослойные сети научились обучаться. Нейросети вернулись из забвения.

**1989 — LeNet (Ян ЛеКун).** Свёрточная нейросеть (CNN) для распознавания почтовых индексов. Первое коммерческое применение — чтение чеков.

**2012 — AlexNet.** Алекс Крижевский выиграл ImageNet с огромным отрывом, обучив CNN на GPU. Запустил современную эру глубокого обучения.

**2017 — Transformer.** Статья «Attention Is All You Need» (Google). Основа всех современных LLM.
...
|-----|--------|-----------|
| **1961** | Stanford Cart | Первый мобильный робот с автономной навигацией. Один «шаг» (1 м) — 10–15 минут вычислений |
